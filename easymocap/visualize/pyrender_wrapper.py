...
class Renderer:
    def __init__(self, bg_color=[1.0, 1.0, 1.0, 0.0], 
                 ambient_light=[0.5, 0.5, 0.5], 
                 flags={},
                 use_lighting=True) -> None:  # ← THÊM DÒNG NÀY
        self.bg_color = bg_color
        self.ambient_light = ambient_light
        self.renderer = pyrender.OffscreenRenderer(1024, 1024)
        self.flags = get_flags(flags)
        self.use_lighting = use_lighting  # ← THÊM DÒNG NÀY
...
def plot_meshes(img, meshes, K, R, T, mode='image', use_lighting=True):
    if use_lighting:
        ambient = [0.5, 0.5, 0.5]
    else:
        ambient = [1.0, 1.0, 1.0]
    renderer = Renderer(use_lighting=use_lighting, ambient_light=ambient)
